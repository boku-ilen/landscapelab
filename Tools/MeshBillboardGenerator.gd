...
func generate_billboard():
	...
	$"1mBillboardDualtex".material_override.albedo_texture = albedo_texture
	
	var normal_texture = _render_to_spritesheet(Viewport.DEBUG_DRAW_NORMAL_BUFFER)
	$"1mBillboardDualtex".material_override.normal_texture = normal_texture
...
func _render_to_spritesheet(debug_draw):
	...
	var total_image = Image.create(1024, 1024, true, front_image.get_format())
	
	total_image.blit_rect(front_image, Rect2i(0, 0, 512, 512), Vector2i(0, 0))
	total_image.blit_rect(side_image, Rect2i(0, 0, 512, 512), Vector2i(512, 0))
