...
func _ready():
	...
	Layers.connect("new_layer",Callable(self,"add_layer"))
	Layers.connect("removed_layer",Callable(self,"remove_layer"))


func add_layer(layer: Layer):
	var new_layer = layer_widget.instantiate()
	new_layer.layer = layer
	new_layer.name = layer.name
	# hocus pocus
	layer_container.add_child(new_layer)
	
	new_layer.connect("translate_to_layer",Callable(position_manager,"set_offset"))
