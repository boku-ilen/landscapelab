extends Control
class_name SpecificLayerCompositionUI

@onready var warning = get_node("RightBox/Warning")

const LOG_MODULE := "LAYERUI"

# Sometimes (e.g. when checking a checkbox) additional content will be shown
# sadly, godot does not react to this automatically, sending a resize signal
# such that the contents are inside the window works tho
signal new_size(add)


func _ready():
	connect("resized",Callable(self,"_on_resize"))


func init(layer_composition: LayerComposition = null):
	if layer_composition != null:
		init_specific_layer_composition_info(layer_composition)


# To be implemented by a child class
func assign_specific_layer_composition_info(layer_composition: LayerComposition):
	pass


# To be implemented by a child class
func init_specific_layer_composition_info(layer_composition: LayerComposition):
	pass


func print_warning(warning_text: String = ""):
	warning.visible = true
	warning.text = warning_text
	logger.warn(warning_text, LOG_MODULE)


func validate(geodata_type):
	return geodata_type and geodata_type.is_valid()


func _on_resize():
	emit_signal("new_size", size)
