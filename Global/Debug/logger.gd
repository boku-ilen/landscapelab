...
# The logger appends all logged data to this array
var stream = []
var stream_mutex = Mutex.new()
var MAX_STREAM_SIZE = 0
...
# We need to get the settings in the _ready function since the logger is instantiated before the settings on startup
func _ready():
	MAX_STREAM_SIZE = Settings.get_setting("logger", "max-messages-in-stream")
	level = Settings.get_setting("logger", "default-level")
...
func _write(type, data):
	...
	# Insert the message into the log stream
	# We must use a mutex here, since the logger can be called from threads!
	stream_mutex.lock()
	stream.push_back(message)
	# Pop old elements if the stream is too big
	while stream.size() > MAX_STREAM_SIZE:
		stream.pop_front()
	stream_mutex.unlock()
