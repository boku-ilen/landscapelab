...
#
# Upstream repo: https://github.com/hedin-hiervard/godot-logger
...
func add_module(name, output_level = default_output_level, \
	...
	if modules.has(name):
		...
		modules[name] = LogModule.new(name, output_level, output_strategies, logfile)
...
func load_config(configfile = default_configfile_path):
	...
	if config.get_value(PLUGIN_NAME, "modules"):
		for module_cfg in config.get_value(PLUGIN_NAME, "modules"):
			var module = LogModule.new(module_cfg["name"], module_cfg["output_level"], \
