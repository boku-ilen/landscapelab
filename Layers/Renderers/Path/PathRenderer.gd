...
func load_new_data():
	...
	for geo_line in geo_lines:
		...
		update_new_line(geo_line, line_vis_instance)
...
func update_new_line(geo_line, line_vis_instance):
	...
	_adjust_height(line_vis_instance)


# Adjust the height to a possible changed terrain
func _adjust_height(line_vis: Path):
	for index in range(line_vis.curve.get_point_count()):
		var point = line_vis.curve.get_point_position(index)
		var adjusted = Vector3(point.x, _get_height_at_ground(point), point.z)
		line_vis.curve.set_point_position(index, adjusted)
